...
# Create a subset for training purpose 
ts_US_train <- ts(US[1:(nobs_US-n_for_US),2:11],
                    start=c(year(US$Month[1]), month(US$Month[1])),
                    frequency = 12)

# Create a subset for testing purpose
start_row_US = nobs_US - n_for_US + 1
ts_US_test <- ts(US[(nobs_US - n_for_US + 1):nobs_US,2:11],
                   start=c(year(US$Month[start_row_US]),
                           month(US$Month[start_row_US])), frequency = 12)
