...
## Forecast Comparison and Best Model Selction
...
## Unemployment Rate Forecast: 2025
...
## Unemployment Model Fitting (United States)
...
```{r time series, echo=FALSE, message=FALSE, warning=FALSE, fig.cap="Time series plots of the training and testing sets", fig.width=8, fig.height=4, results='hide'}
# Transform into time series
ts_US <- ts(US[,2:11],
              start=c(year(US$Month[1]), month(US$Month[1])),
              frequency = 12)
...
### Decomposing the time series for US (Original)  
...
```{r fig:decomp_us, echo=FALSE, warning=FALSE, message=FALSE, fig.cap="Decomposition of the training set", fig.width=8, fig.height=4, results='hide'}
# Decompose 
decom_totalper_trainUS <- decompose(ts_US_train[,"Total.Per"])
plot(decom_totalper_trainUS)
```

To remove the seasonal component, a seasonally adjusted series was generated using the `seasadj()` function.
  

```{r fig:deseas_us, echo=FALSE, warning=FALSE, message=FALSE, fig.cap="Seasonally adjusted U.S. unemployment rate",fig.width=8, fig.height=4, results='hide'}
# Deseason 
deseas_totalper_trainUS <- seasadj(decom_totalper_trainUS)  
...
## Testing Time Series Models for US (Original)  
...
### Performance check for time series models (Original) 
...
### Analysis of the best three models for US (Original)  
...
## Analysis of the best three models for US (Outliers-removed Series)
...
### Transform into time series and set training and testing windows for US (Cleaned)   
...
### Decompose the time series for US (Outliers)  
...
## Testing Time Series Models for US (Cleaned)  
...
## Performance check for US (Cleaned)  
...
```{r Selecting Best Models, echo=FALSE, warning=FALSE, message=FALSE, fig.cap="Forecast Comparison Across Models (Outliers Removed)"}
# Create Tables 
kbl(tpscoresUSout, 
      caption = "Forecast Accuracy for Unemployment Rate (\\%) Data",
...
## Forecast for 2025 with the best three models for US (Cleaned)  
...
```{r Actual VS Forecast, echo=FALSE, warning=FALSE, message=FALSE, fig.width=10, fig.height=5, results='hide', fig.align='center', fig.pos='H', fig.cap="US Unemployment Rate: Actual (2018~) + Avg Forecast (2025)"}
...
1. **Model Performance**: 
   ...
   - **NNETAR** excelled in capturing nonlinear trends for Colombia, while **SARIMA** and **TBATS** emphasized seasonal patterns.  
   - For the U.S., **BSM** and **ETS** provided the most accurate forecasts, leveraging structural and smoothing techniques.  

2. **Developing vs. Developed**:  
   - **Colombia**: Required adaptive models (e.g., NNETAR) to address structural shocks.  
   - **U.S.**: Classical models (e.g., SARIMA) sufficed due to stable labor market institutions.  

3. **Policy Implications**:  
   - **Developing economies** need dual interventions: formalization policies and shock-responsive safety nets.  
   - **Developed economies** should refine automatic stabilizers to maintain labor market resilience.  

4. **Methodological Lessons**:  
   - Ensemble approaches (e.g., averaging NNETAR, SSES, TBATS) reduced forecast error by 5.2% for Colombia.  
   - Outlier removal improved model robustness but required trade-offs in capturing extreme events (e.g., COVID-19).  

This study underscores the importance of tailoring forecasting approaches to economic contexts, with implications for policymakers and researchers navigating labor market uncertainties.  
