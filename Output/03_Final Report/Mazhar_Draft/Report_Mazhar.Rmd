---
title: "Comparative Unemployment Forecasting: US and Colombia"
author: "Mazhar Zuman"
date: "Spring 2025"
output:
  pdf_document:
...
# Define ts_Colombia_train and ts_Colombia_train_out BEFORE using them
ts_Colombia_train <- ts(
  Colombia %>% filter(Month >= as.Date("2010-01-01")) %>% pull(Total.Per),
  start = c(2010, 1), frequency = 12
)

q1 <- quantile(ts_Colombia_train, 0.25, na.rm = TRUE)
q3 <- quantile(ts_Colombia_train, 0.75, na.rm = TRUE)
iqr <- q3 - q1
lower_bound <- q1 - 1.5 * iqr
upper_bound <- q3 + 1.5 * iqr

outliers <- which(ts_Colombia_train < lower_bound | ts_Colombia_train > upper_bound)
ts_Colombia_train_out <- ts_Colombia_train

# Replace outliers and interpolate
if (length(outliers) > 0) {
  ts_Colombia_train_out[outliers] <- NA
  ts_Colombia_train_out <- na.interp(ts_Colombia_train_out)
}
```
...
compare_df <- bind_rows(
  global_ts %>% rename(Total.Per = global_unemployment),
  US %>% select(Month, Total.Per) %>% mutate(Country = "US"),
...
adf_result <- adf.test(ts_Colombia_train_out)
df_temp <- data.frame(Month = time(ts_Colombia_train_out),
                      Value = as.numeric(ts_Colombia_train_out))
df_temp$MonthNum <- cycle(ts_Colombia_train_out)
kruskal.test(Value ~ MonthNum, data = df_temp)
...
- More consistent decomposition structures
- Significantly improved stationarity
- Better Ljung–Box test results
- Lower RMSE and MAPE across most models
